...
// ! Function under test
function sanitizeFruitBasket(inBasket, outBasket) {
  return inBasket.filter((fruit) => fruit !== outBasket);
}

// ! Unit tests (using Jest)
describe('sanitizeFruitBasket', () => {
  test('should take two parameters', () => {
    expect(sanitizeFruitBasket).toHaveLength;
  });
  ...
  test('should return a new array that does not include the unwanted `lemon`', () => {
    const finalBasket = sanitizeFruitBasket(fruitBasket, 'lemon');
    const expected = ['apple', 'grapefruit', 'banana', 'watermelon'];
    expect(finalBasket).toEqual(expected);
  });
});
